...
class DQN(nn.Module):
    ...
    def forward(self, inp):
        im = inp[:,:-2].view(1,-1,self.h,self.w)
        x = self.act_F(self.bn1(self.conv1(im)))
        x = self.act_F(self.bn2(self.conv2(x)))
        x = self.act_F(self.bn3(self.conv3(x)))
        x = torch.cat((x.view(x.size(0), -1),inp[:,-2:]), dim=1) 
        x = self.head(x)
        return self.act_F(x)
